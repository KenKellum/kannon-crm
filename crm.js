...
function _intakeCollectResponses() {
  ...
  for (const id of _intakeChecked) {
    if (id === 'household_members' && window._imHH && window._imHH.length) {
      imHHRead_();
      responses.household_struct = JSON.stringify(window._imHH);
      responses.household_size = String(window._imHH.length);
      responses.member_ages = window._imHH.map(m => m.age).filter(a => a !== '').join(', ');
      continue;
    }
...
async function viewIntakeSession(sessionId) {
  ...
  const { data: s, error } = await supabaseClient
    ...
    .single();
  if (error || !s) {
    showModal('&#128196; Intake Form', '<div style="color:var(--danger);padding:20px;">Could not load intake session.</div>', null, { hideConfirm: true });
    return;
  }
  const typeLabel = INTAKE_TYPE_LABELS[s.form_type] || s.form_type || 'Intake';
  const cName    = (s.contacts && s.contacts.name)    || 'Unknown Contact';
  const cCompany = (s.contacts && s.contacts.company) || '';
  ...
  const body = '<div style="margin-bottom:16px;padding:12px;background:var(--surface-2);border-radius:8px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">'
    ...
    + (cCompany ? '<div style="font-size:12px;color:var(--text-muted);">' + escFn(cCompany) + '</div>' : '')
    + '<div style="font-size:12px;color:var(--text-muted);margin-top:2px;">' + typeLabel + ' &middot; ' + dt + '</div>'
    + '</div>'
    + statusBadge
    + '</div>'
    + fieldHtml;
...
function renderMedigapPicker_() {
  ...
  el.innerHTML = `
    <div style="padding:12px 18px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
      ...
      ${window._qbMgCmp.size ? `<button class="btn ${window._qbMgCmp.size >= 2 ? 'btn-primary' : 'btn-outline'} btn-sm"
        style="margin-left:auto;" ${window._qbMgCmp.size >= 2 ? '' : 'disabled'} onclick="openMgCompare_()">\u2696\ufe0f Compare (${window._qbMgCmp.size})</button>` : ''}
      <button class="btn ${window._qbMgShowGrid ? 'btn-primary' : 'btn-outline'} btn-sm" ${window._qbMgCmp.size ? '' : 'style="margin-left:auto;"'} onclick="mgToggleGrid_()">
        ${window._qbMgShowGrid ? '\u25A4 Show the plans' : '\u25A4 All benefits at once'}</button>
...
// Whether a tool suits THIS client. It never blocks \u2014 an agent may be
// quoting a spouse, or someone under 65 on disability \u2014 but it should not
// take a 55-year-old through a Medicare tool without saying something.
function qbClientAge_() {
  const c = window._qbContact || {};
  const dob = c.date_of_birth || c.dob || window._qbMgDob;
  return dob ? qbAgeFrom_(dob) : null;
}

function qbToolFit_(key) {
  const age = qbClientAge_();
  const medicareish = age != null && age >= 64;
  if (key === 'aca') {
    if (age != null && age >= 65) return { ok: false, why: 'They are ' + age + ' \u2014 most people this age are on Medicare, not a Marketplace plan.' };
    return { ok: true };
  }
  if (key === 'cms' || key === 'rate') {
    if (age == null) return { ok: true, why: 'No date of birth on file \u2014 add one and this will check whether Medicare fits them.' };
    if (!medicareish) return { ok: false, why: 'They are ' + age + ' \u2014 Medicare starts at 65, or earlier on disability.' };
    return { ok: true };
  }
  return { ok: true };
}
  ...
  card.innerHTML = `<div style="background:var(--surface-1);border:0.5px solid var(--border);border-radius:8px;margin-top:10px;overflow:hidden;">
      ...
      ${open ? `<div style="padding:0 12px 10px;">
        ${sources.map(src => {
          ...
          const fit = qbToolFit_(src.key);
          return `<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:5px 0;border-top:1px solid var(--border);${fit.ok ? '' : 'opacity:.72;'}">
            <div style="flex:1;min-width:0;">
              <div style="font-size:12.5px;font-weight:700;">${src.title}${used ? ' <span style="font-size:10px;font-weight:800;color:var(--text-success);">\u2713 RUN</span>' : ''}${
                fit.ok ? '' : ' <span style="font-size:10px;font-weight:800;color:var(--text-warning);">DOESN\u2019T LOOK LIKE A FIT</span>'}</div>
              ...
              ${fit.why ? `<div style="font-size:11px;color:var(--text-warning);margin-top:2px;">${escWeb(fit.why)}</div>` : ''}
            </div>
  ...
  try {
    ...
    if (hhStruct && hhStruct.length && !fromProfile.household) {
      ...
      hhStruct.forEach((m, i) => qbAcaMemberRow_({
        relationship: i === 0 ? 'Self' : (m.relationship === 'Spouse' ? 'Spouse' : 'Child'),
        age: m.age, gender: m.gender === 'F' ? 'F' : (m.gender === 'M' ? 'M' : (c.gender || '')),
        uses_tobacco: !!m.tobacco, applying: m.covered !== false,
        has_mec: false,
      }));
      const covered = hhStruct.filter(m => m.covered !== false).length;
      pulled.push('household (' + hhStruct.length + ' members, ' + covered + ' applying \u2014 from their table)');
    }
    // Household members from the ages list (legacy fallback)
    if (!(hhStruct && hhStruct.length) && r.member_ages) {
...
// ZIP prefix -> state. A ZIP on file is enough to scope a search; we should
// never make an agent wait on a county dropdown to stop seeing other states.
const ZIP3_STATE = [
  [5,9,'PR'],[10,27,'MA'],[28,29,'RI'],[30,38,'NH'],[39,49,'ME'],[50,59,'VT'],
