...
function handleGmailMessage(e){if(e.data&&e.data.type==="gmail-connected")onGmailConnected(e.data.gmail_email||"");}
function onGmailConnected(email){currentAgent.gmail_connected=true;currentAgent.gmail_email=email;const el=document.getElementById("gmail-setup-status");if(el)el.style.display="block";window.removeEventListener("message",handleGmailMessage);setTimeout(()=>{const ov=document.getElementById("gmail-setup-overlay");if(ov)ov.remove();renderDashboard();showToast("Gmail connected! Sending from "+(email||"your Gmail"));},1800);}
function dismissGmailSetup(){const ov=document.getElementById("gmail-setup-overlay");if(ov)ov.remove();window.removeEventListener("message",handleGmailMessage);showToast("Connect Gmail anytime from your dashboard.");}
...
// BOOT
// ============================================================
init();pabaseClient.from('agent_applications').update({
    status: 'denied',
    reviewed_by: currentAgent.id,
    reviewed_at: new Date().toISOString(),
    review_notes: reason || null
